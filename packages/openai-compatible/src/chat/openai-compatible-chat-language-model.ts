...
export type OpenAICompatibleChatConfig = {
  ...
  /**
   * Minimum chunk size (in characters) for streaming responses.
   * Buffers content until this threshold is reached before emitting.
...
export class OpenAICompatibleChatLanguageModel implements LanguageModelV4 {
  ...
  async doStream(
    ...
    const minChunkSize = this.config.minChunkSize ?? 0;
    let textBuffer = '';
    let reasoningBuffer = '';

    const flushTextBuffer = (controller: TransformStreamDefaultController<LanguageModelV4StreamPart>) => {
    ...
    const flushReasoningBuffer = (controller: TransformStreamDefaultController<LanguageModelV4StreamPart>) => {
          ...
          transform(chunk, controller) {
            ...
            if (reasoningContent) {
              ...
              if (minChunkSize > 0) {
                ...
                if (reasoningBuffer.length >= minChunkSize) {
                  flushReasoningBuffer(controller);
                }
              } else {
                controller.enqueue({
                  type: 'reasoning-delta',
